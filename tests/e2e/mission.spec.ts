...
test('completes a real two-run mission and persists progress', async ({ page }, testInfo) => {
  ...
  await expect(page.getByText('Lesson target ready')).toBeVisible()

  await page.locator('.run-field').click({ force: true })
  ...
  await expect(page.getByText('1.00 Mt', { exact: true })).toBeVisible()
  await page.locator('.run-field').click({ force: true })
  await page.getByRole('button', { name: 'Pause' }).click({ force: true })
  await page.getByRole('button', { name: 'Lesson', exact: true }).click()
