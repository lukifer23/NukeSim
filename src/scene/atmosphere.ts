...
export function lightingAmount(day: number): number {
  const u = Math.min(1, Math.max(0, day))
...
export function atmosphereLook(day: number, biome: CityBiome): AtmosphereLook {
  ...
  const dusk = day < 0.34 || day > 0.72
  const night = day < 0.18 || day > 0.92
  ...
  return {
    ...
    sunPos: [2800 + t * 900, 400 + t * 5400, 1600],
